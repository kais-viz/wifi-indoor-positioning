...
inTraining <- createDataPartition(BsubTraining$LATITUDE, p = .80, list = FALSE)
...
tic()
#Creating RF model
rfModel <- train(LATITUDE~., data = training, 
                 method = "rf",
                 trControl=rfControl,
                 tuneLength = 1,
                 #tuneGrid=rfGrid, 
...
postResample(testing$pred, testing$LATITUDE)

BsubTesting$pred <- predict(rfModel, newdata = BsubTesting)
postResample(BsubTesting$pred, BsubTesting$LATITUDE)
confusionMatrix(BsubTesting$pred, BsubTesting$LATITUDE)
...
tic()
knnModel <- train(LATITUDE~. , data = training, method = "knn",
...
#Set seed to know the random order
tic()
SVMModel <- train(LATITUDE ~., data = training, method = "svmLinear",
...
postResample(BsubTesting$pred, BsubTesting$LATITUDE)
#confusionMatrix(B1subTesting$pred, B1subTesting$LATITUDE)
...
tic()
gbmFit <- train(LATITUDE~., data = training, 
                method = "gbm", 
...
tic()
rangerModel <- 
  rand_forest(mode = "regression", mtry = 26) %>%
  set_engine("ranger", seed = 63555) %>%
  fit_xy(x = preds, y = BsubTraining$LATITUDE)
toc()

testing$pred <- predict(rangerModel, testing)
postResample(testing$LATITUDE, unlist(testing$pred))

BsubTesting$pred <- predict(rangerModel, BsubTesting)
postResample(BsubTesting$LATITUDE, unlist(BsubTesting$pred))

library(rgbif)
user <- Sys.getenv("test55555")
elevation(latitude = b1,
          longitude = a1,
          username = user, verbose = TRUE)
GNgtopo30(54,-1)

summary(trainingData$LONGITUDE)
a1<-trainingData[1,]$LONGITUDE
b1<-trainingData[1,]$LATITUDE
a2<-trainingData[2,]$LONGITUDE
b2<-trainingData[2,]$LATITUDE
